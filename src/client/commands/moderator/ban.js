...
module.exports = {
    ...
    cooldown: 30000,//30s;
    permissions: {
        bot: [PermissionsBitField.Flags.BanMembers],
        user: [PermissionsBitField.Flags.BanMembers]
    },
    execute(client, message, args){
        if(BAN_COOLDOWN.has(message.author.id)) return message.channel.send("Wait 30s after using this command to use it again.").catch(() => void(0));
        ...
        db.collection("guilds").findOne({_id: message.guild.id}).then(document => {
            ...
            BAN_COOLDOWN.add(message.author.id);
            const user = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
            if(!user) return message.channel.send("Member not found").catch(() => void(0));
            if(user.permissions.has([PermissionsBitField.Flags.Administrator])) return message.channel.send("You can't ban a administrator.").catch(() => void(0));
            
            const userRoles = user.roles.cache.map(role => role.id);
            const modRoles = document.moderator["roles"];
            if(userRoles.some(roleId => modRoles.includes(roleId))) return message.channel.send("You can't ban a moderator.").catch(() => void(0));
            
            if(user.id === message.author.id) return message.channel.send("You can't ban yourself.").catch(() => void(0));
            ...
            user.ban({reason: reason}).then(member => {
                const Embed = new EmbedBuilder()
                .setColor(RichEmbed.color)
                .setAuthor({ name: `${member.user.tag} has been banned`, iconURL: member.displayAvatarURL({ format: "png", size: 256 })})
                .setDescription(`**Reason:** ${reason}`)

                message.channel.send({embeds: [ Embed ]}).catch(() => void(0));
            }).catch(() => void(0));
        }).catch(() => void(0));
        setTimeout(() => BAN_COOLDOWN.delete(message.author.id), this.cooldown);
    }
};
